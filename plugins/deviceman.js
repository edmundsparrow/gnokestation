...
// Version: 1.0.1 (Fixed collapse arrows)
  ...
  const createDeviceManager = () => {
      ...
      tree.innerHTML = Object.entries(categories).map(([cat, devices]) => `
        <div style="margin: 5px 0;">
          <div style="
            ...
            cursor: pointer;
            user-select: none;
          " onclick="const list = this.nextElementSibling; const arrow = this.querySelector('.arrow'); if(list.style.display === 'none') { list.style.display = 'block'; arrow.textContent = '▼'; } else { list.style.display = 'none'; arrow.textContent = '▶'; }">
            <span class="arrow">▼</span> ${cat} (${devices.length})
          </div>
    ...
    const getCategoryName = (name) => {
      const categories = {
        ...
        serial: 'Serial Ports',
        'web-serial-advanced': 'Serial Ports',
        nfc: 'Near Field Communication',
        gpio: 'System Devices',
        can: 'Industrial Bus Systems',
        'modbus-rtu': 'Industrial Bus Systems',
        'modbus-tcp': 'Industrial Bus Systems',
        profinet: 'Industrial Ethernet',
        mqtt: 'Network Protocols',
        lidar: 'Sensors',
        hid: 'Human Interface Devices',
        'web-android': 'Mobile Devices',
        'web-apple': 'Mobile Devices'
      };
