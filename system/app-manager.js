...
(function () {
  ...
  window.AppManager = {
      ...
      const html = `
        <style>
          ...
          .am-header { 
            ...
            font-size:16px; 
            font-weight:600; 
            border-bottom: 2px solid #1a3c5d;
            display: flex;
        ...
        <div class="am-container">
          <div class="am-header">
            ...
            <button class="am-add-btn" id="am-add-btn" title="Install New App">+</button>
          </div>
    ...
    setup(win) {
      // Setup + button to open installer service (driver-style call)
      const addBtn = win.querySelector('#am-add-btn');
      if (addBtn) {
        addBtn.onclick = () => {
          // Call the installer service directly (like calling a hardware driver)
          if (window.InstallerApp && typeof window.InstallerApp.open === 'function') {
            window.InstallerApp.open();
          } else {
            console.error('[AppManager] InstallerApp service not found');
            alert('Installer service not loaded. Please ensure installer.js is included in your app-loader.js scripts array.');
          }
        };
      }
    ...
    render(win) {
      ...
      // Filter logic with detailed tracking (keeping the tracking logic only internally for the filter)
